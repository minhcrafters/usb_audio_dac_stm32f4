...
fn main() {
    ...
    if args.len() != 3 {
        eprintln!("Usage: {} <COM_PORT> <WAV_FILE_PATH>", args[0]);
        std::process::exit(1);
    }
    ...
    if spec.channels != 2 || spec.sample_rate != 48000 || spec.bits_per_sample != 16 {
        eprintln!("Unsupported WAV file format. Only 48000Hz, 16-bit stereo is supported.");
        return;
    }
    ...
    for frame in 0..output[0].len() {
        samples.push((output[0][frame] * 32767.0) as i16);
        samples.push((output[1][frame] * 32767.0) as i16);
    }
